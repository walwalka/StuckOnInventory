...
const CreateRelicType = () => {
  const [name, setName] = useState('');
  const [loading, setLoading] = useState(false);
  const navigate = useNavigate();
  const { enqueueSnackbar } = useSnackbar();

  const handleSaveRelicType = () => {
    if (!name) {
    ...
    api
      .post('/relictypes', data)
      .then(() => {
        setLoading(false);
        enqueueSnackbar('Relic type created successfully', { variant: 'success' });
        navigate('/relictypes');
      })
      .catch((error) => {
        setLoading(false);
        enqueueSnackbar('Error creating relic type', { variant: 'error' });
        console.log(error);
      });
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-50 p-4 overflow-y-auto">
      <div className='flex flex-col border-2 usd-border-green bg-white dark:bg-[#2c2c2c] rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 mx-auto shadow-2xl relative my-8'>
        ...
        <h1 className='text-3xl mb-6'>Create Relic Type</h1>
        {loading ? <Spinner /> : ''}
        <div className='my-4'>
          ...
          <input
            ...
            placeholder='e.g., Arrowhead, Pottery, Tool'
          />
        </div>
        <button className='p-2 usd-btn-green m-8 rounded hover:opacity-90' onClick={handleSaveRelicType}>
          Save
        </button>
      </div>
    </div>
...
export default CreateRelicType;
