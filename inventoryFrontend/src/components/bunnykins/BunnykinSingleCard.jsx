...
import BunnykinModal from './BunnykinModal';

const BunnykinSingleCard = ({ bunnykin }) => {
    ...
    <div className='border-2 usd-border-silver rounded-lg px-4 py-2 m-4 relative hover:shadow-xl usd-panel'>
            {bunnykin.image1 && (
              <div className='w-full h-48 mb-4 overflow-hidden rounded-lg'>
                <img 
                  src={getImageUrl(bunnykin.image1)} 
                  alt={bunnykin.name} 
      ...
      <div className='flex justify-between items-center gap-x-2 mt-4 p-4'>
        ...
        />
        <Link to={`/bunnykins/details/${bunnykin.id}`}>
          <BsInfoCircle className='text-2xl text-green-800 hover:text-black' />
        </Link>
        <Link to={`/bunnykins/edit/${bunnykin.id}`}>
          <AiOutlineEdit className='text-2xl text-yellow-600 hover:text-black' />
        </Link>
        <Link to={`/bunnykins/delete/${bunnykin.id}`}>
          <MdOutlineDelete className='text-2xl text-red-600 hover:text-black' />
        </Link>
      </div>
      {showModal && (
        <BunnykinModal bunnykin={bunnykin} onClose={() => setShowModal(false)} />
      )}
    </div>
  );
};

export default BunnykinSingleCard;
