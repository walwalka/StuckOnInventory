...
const ComicPublishersTable = ({ publishers }) => {
  const [selectedPublisher, setSelectedPublisher] = useState(null);

  const handleDetailsClick = (publisher) => {
        ...
        <table className='w-full border-collapse'>
          ...
          <tbody className='bg-white dark:bg-[#3c3c3c] divide-y divide-gray-200 dark:divide-stone-700'>
            {publishers.map((publisher, index) => (
              <tr key={publisher.id} className='hover:bg-gray-50 dark:hover:bg-[#4a4a4a] transition-colors'>
                ...
                <td className='px-4 py-3'>
                  <div className='flex justify-center gap-x-3'>
                    <button onClick={() => handleDetailsClick(publisher)} className="hover:scale-110 transition-transform">
                      <BsInfoCircle className='text-xl' style={{ color: 'var(--usd-green)' }} />
                    </button>
                    <Link to={`/comicpublishers/edit/${publisher.id}`} className="hover:scale-110 transition-transform">
                      <AiOutlineEdit className='text-xl' style={{ color: 'var(--usd-copper)' }} />
                    </Link>
                    <Link to={`/comicpublishers/delete/${publisher.id}`} className="hover:scale-110 transition-transform">
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/30 backdrop-blur-sm z-50 p-4">
          <div className='flex flex-col border-2 usd-border-green bg-white dark:bg-[#2c2c2c] rounded-xl max-w-md w-full p-6 shadow-2xl'>
            <div className="flex items-center justify-between mb-4">
              <h1 className='text-2xl usd-text-green font-semibold'>Comic Publisher Details</h1>
              <button
                onClick={closeModal}
            ...
            <div className='my-2'>
              ...
              <span className='text-gray-900 dark:text-gray-100'>{selectedPublisher.id}</span>
            </div>
            <div className='my-2'>
              <span className='text-xl mr-4 usd-muted'>Name</span>
              <span className='text-gray-900 dark:text-gray-100'>{selectedPublisher.name}</span>
            </div>
...
export default ComicPublishersTable;
